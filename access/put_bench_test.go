...
func BenchmarkFlatFields_PaospPackableComputBench2(b *testing.B) {
	const count = 1000
	x := map[string]string{
		"user":  "alice",
		"role":  "admin",
	...
	perPack := float64(elapsed.Nanoseconds()) / float64(b.N*count)
	opsPerSec := 1e9 / perPack
	b.Logf("PaospPackableComput2: per-pack = %.2f ns/op, %.2f ops/sec", perPack, opsPerSec)
	b.Logf("PaospPackableComput2 size: %d bytes", len(sinkFlat))
