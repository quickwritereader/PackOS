...
func TestPackable_TestWithOrderedMaps(t *testing.T) {
	...
	actual := Pack(
		...
		PackMapOrdered(
			PP("meta",
				PackMapOrdered(
					PP("role", PackByteArray([]byte("admin"))),
					PP("user", PackByteArray([]byte("alice"))),
				),
			),
			PP("name", PackString("gopher")),
		),
	)
...
func TestPackable_TestPutAccessWithPackOrdered(t *testing.T) {
	mapx := PackMapOrdered(
		PP("meta",
			PackMapOrdered(
				PP("role", PackByteArray([]byte("admin"))),
